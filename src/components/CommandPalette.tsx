        ...
        <CommandGroup heading="Pages">
          {pages.map((p) => (
            <CommandItem
              key={p.id}
              value={`page ${p.title}`}
              onSelect={() => go(() => navigate({ to: "/pages/$id", params: { id: p.id } }))}
            >
        ...
        <CommandGroup heading="Boards">
          {boards.map((b) => (
            <CommandItem
              key={b.id}
              value={`board ${b.name}`}
              onSelect={() => go(() => navigate({ to: "/boards/$id", params: { id: b.id } }))}
            >
          ...
          {cards.slice(0, 30).map((c) => {
            ...
            return (
              <CommandItem
                ...
                value={`task ${c.title}`}
                onSelect={() =>
                  go(() => list && navigate({ to: "/boards/$id", params: { id: list.boardId } }))
                }
              >
                <LayoutGrid className="h-3.5 w-3.5 mr-2 text-muted-foreground" />
                {c.title}
        ...
        <CommandGroup heading="Events">
          {events.map((e) => (
            <CommandItem
              key={e.id}
              value={`event ${e.title}`}
              onSelect={() => go(() => navigate({ to: "/calendar" }))}
            >
              <CalIcon className="h-3.5 w-3.5 mr-2 text-muted-foreground" />
              {e.title}
            </CommandItem>
          ))}
        </CommandGroup>
