...
export function Sidebar({
  ...
  const favorites = [
    ...pages
      .filter((p) => p.favorite)
      .map((p) => ({ kind: "page" as const, id: p.id, name: p.title, icon: p.icon })),
    ...boards
      .filter((b) => b.favorite)
      .map((b) => ({ kind: "board" as const, id: b.id, name: b.name, icon: b.icon })),
  ];
    ...
    <div className="flex flex-col h-full text-sidebar-foreground">
      ...
      <nav className="px-2 space-y-0.5 mt-1">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-2 py-1 text-sm rounded hover:bg-sidebar-hover"
        >
        ...
        <NavItem
          ...
          label="Inbox"
          badge={notifs > 0 ? notifs : undefined}
        />
      </nav>
      ...
      <div className="flex-1 overflow-y-auto mt-3 px-2 space-y-3">
        {favorites.length > 0 && (
          <Section title="Favorites">
            {favorites.map((f) => (
              <Link
                key={f.id}
                to={f.kind === "page" ? "/pages/$id" : "/boards/$id"}
                params={{ id: f.id }}
        ...
        >
          {rootPages.map((p) => (
            <PageTree key={p.id} page={p} pages={pages} depth={0} />
          ))}
          {rootPages.length === 0 && (
            <div className="text-xs text-muted-foreground px-2 py-1">No pages</div>
          )}
        </Section>
...
function IconBtn({ icon, onClick }: { icon: React.ReactNode; onClick: () => void }) {
  return (
    <button onClick={onClick} className="p-2 rounded hover:bg-sidebar-hover">
      {icon}
    </button>
  );
}
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      {badge ? (
        <span className="ml-auto text-[10px] bg-primary text-primary-foreground rounded-full px-1.5">
          {badge}
        </span>
      ) : null}
    </Link>
...
function PageTree({ page, pages, depth }: { page: Page; pages: Page[]; depth: number }) {
  ...
  return (
    <div>
      <div
        className={cn(
          "group flex items-center gap-1 px-1 py-1 text-sm rounded hover:bg-sidebar-hover",
      ...
      >
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-sidebar-active rounded">
              <MoreHorizontal className="h-3.5 w-3.5" />
            </button>
          </PopoverTrigger>
          <PopoverContent className="w-44 p-1" align="start">
            <MenuItem onClick={() => actions.toggleFavorite("page", page.id)}>
              {page.favorite ? "Unfavorite" : "Add to Favorites"}
            </MenuItem>
            <MenuItem
              onClick={() => {
                const id = actions.createPage(page.id);
                navigate({ to: "/pages/$id", params: { id } });
              }}
            >
              Add sub-page
            ...
            <MenuItem
              onClick={() => {
                ...
                if (newTitle != null) actions.updatePage(page.id, { title: newTitle });
              }}
            >
              Rename
            </MenuItem>
            <MenuItem destructive onClick={() => actions.deletePage(page.id)}>
              Delete
            </MenuItem>
          </PopoverContent>
        </Popover>
      ...
      {open &&
        children.map((c) => <PageTree key={c.id} page={c} pages={pages} depth={depth + 1} />)}
    </div>
  );
}
