  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-6 pt-6 pb-3 flex items-center gap-2 flex-wrap">
        ...
        <div className="flex">
          <button
            onClick={() => shift(cursor, view, -1, setCursor)}
            className="p-1.5 border rounded-l-md hover:bg-accent"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <button
            onClick={() => shift(cursor, view, 1, setCursor)}
            className="p-1.5 border-y border-r rounded-r-md hover:bg-accent"
          >
      ...
      <div className="flex-1 overflow-auto px-6 pb-6">
        {view === "month" && (
          <MonthView
            cursor={cursor}
            events={allEvents}
            onEventClick={(id) => events.find((e) => e.id === id) && setEditing(id)}
            onDayClick={(d) => {
...
function MonthView({
  ...
  for (let i = 0; i < startDay; i++)
    cells.push(new Date(cursor.getFullYear(), cursor.getMonth(), i - startDay + 1));
    ...
    <div className="border rounded-lg overflow-hidden">
      <div className="grid grid-cols-7 bg-muted text-xs font-medium">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div key={d} className="px-2 py-1.5 border-r last:border-r-0">
            {d}
          </div>
        ))}
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div
              ...
              onDrop={() => {
                if (dragId) {
                  ...
                  if (ev && !ev.id.startsWith("task-")) {
                    ...
                    newStart.setHours(ds.getHours(), ds.getMinutes(), 0, 0);
              ...
              className={cn(
                "border-r border-b last:border-r-0 min-h-[88px] p-1 text-xs cursor-pointer hover:bg-accent/40",
                !inMonth && "text-muted-foreground/50 bg-muted/20",
              )}
            >
              <div
                className={cn(
                  "inline-flex items-center justify-center h-6 w-6 rounded-full mb-1",
              ...
              <div className="space-y-0.5">
                {dayEvents.slice(0, 3).map((e) => (
                  <div
                    key={e.id}
                    draggable={!e.id.startsWith("task-")}
                    onDragStart={(ev) => {
                      ev.stopPropagation();
                      setDragId(e.id);
                    }}
                    ...
                    className="truncate px-1.5 py-0.5 rounded text-white text-[11px]"
    ...
    <div className="border rounded-lg overflow-hidden">
      <div className="grid" style={{ gridTemplateColumns: `60px repeat(${days.length}, 1fr)` }}>
        ...
        {days.map((d) => (
          <div
            key={d.toISOString()}
            className="border-r border-b last:border-r-0 bg-muted text-xs font-medium p-2 text-center"
          >
        ...
        {hours.flatMap((h) => [
          <div
            key={`h-${h}`}
            className="border-r border-b text-[10px] text-muted-foreground p-1 text-right"
          >
            {h}:00
          </div>,
          ...days.map((d, di) => {
            ...
            return (
              <div
                key={`c-${h}-${di}`}
                className="border-r border-b last:border-r-0 min-h-[44px] p-0.5 hover:bg-accent/30 relative"
              >
                {slotEvents.map((e) => (
                  <div
                    key={e.id}
                    onClick={() => onEventClick(e.id)}
                    className="text-[11px] px-1.5 py-0.5 rounded text-white truncate cursor-pointer mb-0.5"
                    style={{ background: e.color }}
                  >
                    {e.title}
                  </div>
...
function AgendaView({
  ...
  return (
    <div className="space-y-1">
      {list.length === 0 && (
        <div className="text-sm text-muted-foreground p-4">No events this month</div>
      )}
      {list.map((e) => (
        <button
          key={e.id}
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <input
          value={ev.title}
          onChange={(e) => actions.updateEvent(ev.id, { title: e.target.value })}
        ...
        <div className="space-y-3 mt-4 text-sm">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                actions.updateEvent(ev.id, { start: new Date(e.target.value).getTime() })
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                actions.updateEvent(ev.id, { end: new Date(e.target.value).getTime() })
              }
              className="w-full border rounded px-2 py-1 bg-background"
            />
          </div>
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!ev.allDay}
              onChange={(e) => actions.updateEvent(ev.id, { allDay: e.target.checked })}
            />
            All-day
          </label>
          <div>
            ...
            <textarea
              ...
              onChange={(e) => actions.updateEvent(ev.id, { description: e.target.value })}
              rows={3}
          ...
          <div>
            ...
            <div className="flex gap-1.5">
              {EVENT_COLORS.map((c) => (
                <button
                  key={c}
                  onClick={() => actions.updateEvent(ev.id, { color: c })}
                  className={cn(
                    "h-6 w-6 rounded-full border-2",
                    ev.color === c ? "border-foreground" : "border-transparent",
                  )}
                  style={{ background: c }}
                />
              ))}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-1 text-xs text-destructive border border-destructive/30 rounded px-2 py-1.5 hover:bg-destructive/10"
          >
