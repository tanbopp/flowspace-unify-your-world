...
export function CardModal({ cardId, onClose }: { cardId: string; onClose: () => void }) {
  ...
  const checkPct = card.checklist.length
    ? Math.round((checkDone / card.checklist.length) * 100)
    : 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr,180px] gap-6 mt-4">
                ...
                onClick={() => {
                  if (newComment.trim()) {
                    actions.updateCard(card.id, {
                      comments: [
                        ...card.comments,
                        { id: uid(), text: newComment.trim(), at: Date.now() },
                      ],
                    });
                    setNewComment("");
                  }
                }}
                className="mt-1 text-xs bg-primary text-primary-foreground px-2 py-1 rounded"
              >
                Comment
              </button>
            </Section>
          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <select
                value={card.listId}
                onChange={(e) => actions.updateCard(card.id, { listId: e.target.value })}
            ...
            <div>
              ...
              <select
                value={card.priority ?? ""}
                onChange={(e) =>
                  actions.updateCard(card.id, { priority: (e.target.value || undefined) as any })
                }
                className="w-full border rounded px-2 py-1 bg-background"
              >
            ...
            <div>
              ...
              <input
                type="datetime-local"
                value={card.due ? toLocalInput(card.due) : ""}
                onChange={(e) =>
                  actions.updateCard(card.id, {
            ...
            <div>
              ...
              <input
                value={card.assignee ?? ""}
                onChange={(e) =>
                  actions.updateCard(card.id, { assignee: e.target.value || undefined })
                }
                placeholder="Initials"
                maxLength={3}
                className="w-full border rounded px-2 py-1 bg-background uppercase"
              />
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1 mb-1">
                {card.labels.map((l, i) => (
                  <span
                    key={i}
                    className="text-[10px] px-1.5 py-0.5 rounded-full text-white flex items-center gap-1"
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        actions.updateCard(card.id, {
                          labels: card.labels.filter((_, idx) => idx !== i),
                        })
                      }
                    >
                    ...
                    if (e.key === "Enter" && newLabel.trim()) {
                      actions.updateCard(card.id, {
                        labels: [
                          ...
                          {
                            name: newLabel.trim(),
                            color: LABEL_COLORS[card.labels.length % LABEL_COLORS.length],
                          },
                        ],
                      });
                      setNewLabel("");
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="text-xs uppercase text-muted-foreground font-medium mb-2">{title}</div>
      {children}
    </div>
  );
}
